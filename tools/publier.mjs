 ...
 * autorisations.
 *
 * Aucune dépendance : tout passe par `fetch` et le module `crypto` de
 * Node, comme dans `server/`.
 */
...
/**
 * Le nom de paquet vient d'`app.json` : le répéter en argument ouvrirait
 * la porte à publier un jeu sous l'identifiant d'un autre.
 */
function lireNomDePaquet() {
  const racine = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
  const { expo } = JSON.parse(readFileSync(path.join(racine, 'app.json'), 'utf8'));
  const paquet = expo?.android?.package;
  if (!paquet) {
    console.error("app.json ne contient pas expo.android.package.");
    process.exit(1);
  }
  return paquet;
}
